...
namespace AkaryakıtOtomasyon
{
    public partial class Form1 : Form
    {
        ...
        double D_benzin95 = 0, D_benzin97, D_dizel = 0, D_eurodizel = 0, D_lpg = 0;  //depodaki yakıt miktarlarının tutulduğu yer
        double E_benzin95 = 0, E_benzin97, E_dizel = 0, E_eurodizel = 0, E_lpg = 0;  //eklenen değişkenlerin miktarlarının tutulduğu yer
        double F_benzin95 = 0, F_benzin97, F_dizel = 0, F_eurodizel = 0, F_lpg = 0;  //fiyatların tutulduğu değişkenler
        double S_benzin95 = 0, S_benzin97, S_dizel = 0, S_eurodizel = 0, S_lpg = 0;  //satılan değişken miktarlarının tutulduğu yer
        string[] depo_bilgileri;
        string[] fiyat_bilgileri;

        private void button2_Click(object sender, EventArgs e)
        {
            S_benzin95 = double.Parse(numericUpDown1.Value.ToString()); //değişkenlere numericupdowndaki değerleri aktardık
            ...
            if (numericUpDown1.Enabled == true)
            {
                ...
                label40.Text = Convert.ToString(S_benzin95 * F_benzin95);
            }
            else if (numericUpDown2.Enabled == true)
            {
                D_benzin97 = D_benzin97 - S_benzin97;
                label40.Text = Convert.ToString(S_benzin97 * F_benzin97);
            }
            else if (numericUpDown3.Enabled == true)
            {
                D_dizel = D_dizel - S_dizel;
                label40.Text = Convert.ToString(S_dizel * F_dizel);
            }
            else if (numericUpDown4.Enabled == true)
            {
                D_eurodizel = D_eurodizel - S_eurodizel;
                label40.Text = Convert.ToString(S_eurodizel * F_eurodizel);
            }
            else if (numericUpDown5.Enabled == true)
            {
                D_lpg = D_lpg - S_lpg;
                label40.Text = Convert.ToString(S_lpg * F_lpg);
            }
            ...
            numericUpDown1.Value = 0; //her satış yapıldığında butonların sıfırlanmasını sağlar
        ...
        {
            //yakıt türü seçimine göre alttaki seçeneklerin aktifliğinin belirlenmesi:
            ...
            numericUpDown1.Value = 0; //numericlerden biri girildiğinde diğerleri sıfırlanacak
            numericUpDown2.Value = 0;
            numericUpDown3.Value = 0;
            numericUpDown4.Value = 0;
            numericUpDown5.Value = 0;
            label40.Text = "___________";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                F_benzin97 = F_benzin97 + (F_benzin95 * Convert.ToDouble
                    (textBox9.Text) / 100);
            ...
            System.IO.File.WriteAllLines(Application.StartupPath + "\\fiyat.txt", fiyat_bilgileri); //yeni girilen değerler dosyada ve butonlarda güncellendi
            txt_fiyat_oku();
            txt_fiyat_yaz();
        }
        ...
        {
            //textboxlara belli sayılar arasında değer verilmek isteniyor:
            try
            {
            ...
            try
            {
                ...
                if (1000 < D_benzin95 + E_benzin97 || E_benzin97 <= 0)
                    textBox2.Text = "Hata!";
                else
                    depo_bilgileri[1] = Convert.ToString(D_benzin97);
            }
            ...
            System.IO.File.WriteAllLines(Application.StartupPath + "\\depo.txt", depo_bilgileri); //yeni girilen değerler dosyada ve butonlarda güncellendi
            txt_depo_oku();
            txt_depo_yaz();
            progressBar_guncelle();
            numericupdown_value();
        ...
        {
            depo_bilgileri = System.IO.File.ReadAllLines(Application.StartupPath + "\\depo.txt");  //txt dosyasındaki bilgileri okuyup depo_bilgileri dizisine aktardı
            D_benzin95 = Convert.ToDouble(depo_bilgileri[0]);  //ilk elemanı double veri türüne dönüştürecek
        ...
        private void txt_depo_yaz() //depodaki bilgileri label nesnelerine yazdırma methodu
        {
            label28.Text = D_benzin95.ToString("N"); //virgülden sonraki basamak sayısını ikiye ayarlıyor n karakteri
            label25.Text = D_benzin97.ToString("N");
            label31.Text = D_dizel.ToString("N");
            label41.Text = D_eurodizel.ToString("N");
            label26.Text = D_lpg.ToString("N");
        }
        ...
        private void txt_fiyat_yaz() //depodaki bilgileri label nesnelerine yazdırma methodu
        {
            label18.Text = F_benzin95.ToString("N"); //virgülden sonraki basamak sayısını ikiye ayarlıyor n karakteri
            label15.Text = F_benzin97.ToString("N");
            label21.Text = F_dizel.ToString("N");
        ...
        { //satış yapma kısmındaki numericupdownlar depodaki miktarların üstüne çıkamayacak
        ...
        {
            this.Text = "Akaryakıt Otomasyonu"; //form başlığı
            progressBar1.Maximum = 1000; //progressbarların en fazla kaça bölüneceği
            progressBar2.Maximum = 1000;
            progressBar3.Maximum = 1000;
            progressBar4.Maximum = 1000;
            progressBar5.Maximum = 1000;
            txt_depo_oku(); //methodları çağırarak içlerinde yapılan değişiklikleri yazdırma
            txt_depo_yaz();
            txt_fiyat_oku();
            txt_fiyat_yaz();
            progressBar_guncelle();
            ...
            string[] yakit_turleri = { "Benzin (95) ", " Benzin (97) ", "Dizel", "Eurodizel", "LPG" }; //yakıt türleri bölümüne dizi şeklinde değişkenler tanımlandı
            comboBox1.Items.AddRange(yakit_turleri); //dizideki değişkenler comboboxa eklendi
            //form ilk açıldığında bütün numericupdownlar pasif:
            numericUpDown1.Enabled = false;
            numericUpDown2.Enabled = false;
            numericUpDown3.Enabled = false;
            numericUpDown4.Enabled = false;
            ...
            numericUpDown1.DecimalPlaces = 2;  //numericupdownların virgülden sonra 2 basamağı olsun
            numericUpDown2.DecimalPlaces = 2;
            numericUpDown3.DecimalPlaces = 2;
            numericUpDown4.DecimalPlaces = 2;
            numericUpDown5.DecimalPlaces = 2;

            //numericupdownlar yukarı aşağı oka tıklandığında kaçar kaçar artmalı:
            numericUpDown1.Increment = 0.1M; //ondalık olduğunda m yazılır
            numericUpDown2.Increment = 0.1M;
            numericUpDown3.Increment = 0.1M;
            numericUpDown4.Increment = 0.1M;
            numericUpDown5.Increment = 0.1M;

            //numericupdownlara dışarıdan veri girişi yapılmasın:
            numericUpDown1.ReadOnly = true; // yalnızca arttırma va azaltma var dışarıdan veri girişi yok
